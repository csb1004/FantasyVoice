"""A durable boundary encompasses every GAN/Predictor optimizer in an update."""
import math
from pathlib import Path

import torch

from .predictor import atomic_checkpoint, rng_state, restore_rng
...
def fit_engine(modules, optimizers, schedulers, objective, size, config, output,
    ...
    state = {'epoch': 0, 'cursor': 0, 'updates': 0, 'pending_eval': None,
             'best_mel': None, 'best_update': None, 'history': []}
        ...
        while state['cursor'] < size:
            ...
            if progress:
                progress({'updates':state['updates'], 'total_updates':total_updates,
                          'epoch':state['epoch'], 'losses':totals})
